...
> **Focus:** Cloud Architecture · DevSecOps · Infrastructure as Code · Kubernetes · Serverlessdod/Users/pushparajnaik/Desktop/TerraformCode/AWS_DevOps_K8s_clean/README.md/Users/pushparajnaik/Desktop/TerraformCode/AWS_DevOps_K8s_clean/README.md AI · Multi-Cloud · IoT Edge Computing · AWS Landing Zonees
...
This mono-repository is a curated collection of **18 production-grade cloud infrastructure projects** that showcase end-to-end cloud architecture, DevSecOps pipelines, Kubernetes orchestration, serverless AI solutions, multi-cloud platforms, AWS Landing Zone governance, data engineering, and IoT edge computing — all implemented with **Infrastructure as Code (Terraform)** and **GitOps** best practices.
...
### 12. 🏛️ [AWS Landing Zone — Control Tower AFT](./aws-landing-zone-aft/)

> Production-grade AWS Landing Zone using Control Tower Account Factory for Terraform (AFT) — automated multi-account provisioning, IAM Identity Center (SSO) with mandatory TOTP MFA, 6 SCPs, 3-layer Policy-as-Code, and full CI/CD with OIDC.

| Aspect                      | Details                                                                                                                     |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| **Architecture**      | 4 OUs (Security, Shared, Billing, Workload) with 5+ managed accounts                                                        |
| **Account Factory**   | AWS Control Tower AFT — automated account creation via Terraform                                                           |
| **SSO + MFA**         | IAM Identity Center with 6 permission sets, 5 groups, mandatory TOTP (Google/Microsoft Authenticator) — no MFA = no access |
| **Service Catalog**   | `AWSServiceCatalogEndUserAccess` permission set for Control Tower Account Factory console operations                        |
| **Governance**        | 6 SCPs: deny-root, deny-leave-org, restrict-regions, deny-s3-public, require-imdsv2, mandatory-tags                         |
| **Policy-as-Code**    | 3-layer: Static (Checkov + tfsec) → Plan-time (OPA/Conftest, 5 .rego files) → Runtime (SCPs + Config + GuardDuty)         |
| **Security Baseline** | IAM password policy, S3 BPA, EBS encryption, IMDSv2, Config recorder, cross-account audit role                              |
| **Networking**        | VPC CIDR 10.15.0.0/16, per-account /20 subnets (16 accounts), HA NAT GWs (prod), flow logs                                 |
| **CI/CD**             | GitHub Actions with OIDC — plan on PR (with policy gates), apply on merge                                                  |
| **Scaling**           | Map-driven SSO + account requests — add a new account by editing 2 files, zero code changes                                 |
| **IaC**               | 6 modules: VPC, SCP, IAM-Baseline, IAM-Identity-Center, CloudTrail, Security-Baseline                                       |
| **Deployment**        | Day 0 (manual CT) → Day 1 (bootstrap) → Day 2 (AFT) → Day 3 (accounts + SSO)                                             |

**Key Principle:** Day 0 (enable Control Tower in console) is the **only manual step**. Everything else — from state management to account creation to MFA enforcement — is fully automated via Terraform and Make targets. Adding a new account takes ~35 minutes with zero console clicks.

---

### 13. 🎬 [Netflix Clone — DevSecOps Pipeline on AWS EKS](./netflix-clone-devsecops/)

> Production-grade Netflix Clone deployed on AWS EKS with a 10-stage DevSecOps CI/CD pipeline powered by GitHub Actions, replacing Jenkins architecture.

| Aspect | Details |
|--------|---------|
| **Pipeline** | 10-stage CI: Gitleaks → SonarCloud → npm audit → Docker build → Trivy → ECR push |
| **GitOps** | ArgoCD watches Git manifests repo, auto-syncs deployments |
| **Compute** | Private EKS cluster with bastion-only API access |
| **Security** | OIDC (keyless), private subnets, IMDSv2, KMS encryption |
| **Networking** | ALB Ingress, Route53, ACM TLS, VPC 10.0.0.0/16 |
| **CI/CD** | GitHub Actions with OIDC → ECR → Git tag update → ArgoCD |

---

### 14. 🏥 [HealthCloud Multi-Region DR Platform](./healthcloud-multiregion-dr/)

> HIPAA-compliant healthcare platform with AWS (primary) + Azure (DR), Route53 failover, cross-cloud VPN, and fully automated DR testing.

| Aspect | Details |
|--------|---------|
| **Multi-Cloud** | AWS us-east-1 (primary) + Azure eastus (DR) with IPSec VPN |
| **Compute** | EKS 1.30 (primary) + AKS 1.30 (DR) |
| **Database** | Aurora PostgreSQL (primary) ↔ Azure PG Flexible Server (async replication) |
| **DR** | Route53 failover routing, automated DR failover tests |
| **Security** | CloudFront + WAF v2 (AWS), Front Door + WAF (Azure), PHI encryption at rest |
| **Compliance** | HIPAA, SOC 2 aligned — audit logging, encrypted PHI in S3/Blob |

---

### 15. 🏥 [MedCloud Global Platform](./medcloud-global-platform/)

> Multi-Cloud Healthcare & Medical E-Commerce Platform spanning AWS, Azure, and GCP with advanced DevSecOps, AI/ML, and zero-trust security.

| Aspect | Details |
|--------|---------|
| **Multi-Cloud** | AWS (core app + e-commerce) + Azure (AI + medical imaging) + GCP (analytics + ML Ops) |
| **Compute** | EKS + AKS + GKE with Kubernetes federation |
| **Database** | Aurora Global + Cosmos DB + BigQuery + Vertex AI |
| **Security** | Zero-trust architecture, HIPAA/GDPR/PCI-DSS compliant |
| **E-Commerce** | Medical supplies marketplace with payment processing |

---

### 16. 🌐 [EKS Retail Platform](./eks-retail-platform/)

> Production-grade Kubernetes platform on AWS EKS with dual autoscaling (KEDA + Native HPA), Karpenter, Istio service mesh, ArgoCD GitOps, and PCI-DSS/SOC2/HIPAA compliance.

| Aspect | Details |
|--------|---------|
| **Compute** | AWS EKS with Karpenter node management |
| **Autoscaling** | Dual: KEDA (event-driven) + Native HPA (CPU/memory) |
| **Service Mesh** | Istio with STRICT mTLS, traffic management, observability |
| **GitOps** | ArgoCD with automated drift detection and self-healing |
| **Microservices** | storefront-api, order-svc, inventory — retail domain |
| **Compliance** | PCI-DSS, SOC2, HIPAA, GDPR security controls |

---

### 17. 🤖 [Ansible Configuration Management at Scale](./ansible-config-mgmt/)
...
| Aspect | Details |
|--------|---------|
| **Tool** | Ansible with modular roles and playbooks |
| **Scope** | Server provisioning, security hardening, app deployment |
| **Environments** | Multi-environment (dev, staging, prod) inventory management |
| **Security** | CIS benchmark hardening, SSH key rotation, firewall rules |
| **Idempotency** | Fully idempotent playbooks — safe to re-run |

---

### 18. 📊 [Enterprise Multi-Cloud Medallion Data Platform](./medallion-multicloud-platform/)

> Active-Passive Pilot Light data platform (AWS Primary → Azure DR) with Databricks Unity Catalog, Medallion architecture (Bronze/Silver/Gold), and HIPAA/SOC2/PCI-DSS compliance.

| Aspect | Details |
|--------|---------|
| **Architecture** | Medallion: Bronze (raw) → Silver (cleansed) → Gold (business) |
| **Multi-Cloud** | AWS (active) + Azure (passive pilot light DR) |
| **Data Engine** | Databricks with Unity Catalog — RLS, column masking |
| **Security** | Private Link only, SSE-KMS encryption, VPC endpoints, tokenized PII |
| **Compliance** | HIPAA, SOC 2, PCI-DSS — immutable audit trail |
| **IaC** | Terraform + Databricks Asset Bundles |
| **CI/CD** | GitHub Actions with OIDC — validate → test → deploy |
...
| **AWS Control Tower + AFT**   | aws-landing-zone-aft                                                                                         | Multi-account governance, account factory    |
| **IAM Identity Center**       | aws-landing-zone-aft                                                                                         | SSO with MFA enforcement across all accounts |
| **AWS Organizations + SCPs**  | aws-landing-zone-aft                                                                                         | OU hierarchy, service control policies       |
...
| Technology                       | Used In                                                                                          | Purpose                                        |
| -------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| **Checkov**                | aws-cost-anomaly-detector, bedrock-rag-chatbot, multicloud-clearing-engine, aws-landing-zone-aft | IaC security scanning                          |
| **tfsec**                  | aws-landing-zone-aft                                                                             | Terraform-specific security scanning           |
| **OPA/Conftest**           | aws-landing-zone-aft                                                                             | Plan-time policy validation (5 .rego policies) |
...
| **OIDC Federation**     | java-eks-devsecops-pipeline, omnichannel-ai-support-platform, aws-landing-zone-aft                             | GitHub Actions → AWS STS — keyless, credential-free CI/CD                         |
| **Autonomous AI Agent** | claude-code-devops-workflows                                                                                   | AI-driven DevOps automation with quality gates and audit logging                    |
| **Landing Zone**        | aws-landing-zone-aft                                                                                           | Multi-account governance with Control Tower AFT, SSO+MFA, SCPs, Policy-as-Code      |
...
| aws-landing-zone-aft            | ~$0 (AFT)                                                        | Control Tower AFT is free; costs are in vended accounts |
