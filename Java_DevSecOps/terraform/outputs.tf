# ==============================================================================
# Terraform Outputs — Production Architecture (GitHub Actions + EKS)
# ==============================================================================
# Query with: terraform output <name> or terraform output -json
# ==============================================================================
...
output "api_gateway_invoke_url" {
  description = "API Gateway invoke URL (requires Cognito JWT for /api/* routes)"
  value       = aws_apigatewayv2_api.app.api_endpoint
}

output "api_url" {
  description = "API URL via Route53"
...
output "cognito_client_id" {
  description = "Cognito App Client ID (use in your application)"
...
output "audit_logs_bucket" {
  description = "S3 bucket storing CloudTrail audit logs (7-year retention)"
  value       = aws_s3_bucket.audit_logs.bucket
}

output "security_alerts_topic_arn" {
  description = "SNS topic ARN for security alarms — subscribe additional endpoints here"
...
output "compliance_summary" {
  ...
  value       = <<-EOT
    Compliance Controls Active:
      ...
      PCI DSS Req 6  : WAF OWASP rules + Trivy + SonarCloud + OWASP DC
      ...
      SOC 2 CC9      : S3 versioning + CloudTrail log file validation

    Audit Logs:    s3://${aws_s3_bucket.audit_logs.bucket}
    Alerts SNS:    ${aws_sns_topic.security_alerts.arn}
